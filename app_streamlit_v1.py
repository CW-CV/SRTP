...
class UIComponents:
    ...
    def display_compare_view():
        ...
        with col1:
            st.subheader("学生答案")
            st.text_area("学生答案", st.session_state.full_answer, height=400)
        with col2:
            st.subheader("标准答案")
            st.text_area("标准答案", st.session_state.full_standard, height=400)
                ...
                if st.button("🔄 解析新答题卡",
                    ...
                    try:
                        ...
                        if not answer_content:
                            raise ValueError("答题卡内容为空")

                        st.session_state.full_answer = answer_content
                        st.session_state.uploaded_files['answers'] = []  # 清空已处理文件
                        st.success("答题卡解析成功！")
